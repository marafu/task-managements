...
import { CancellingTask } from "../../src/application/usecases/CancellingTask";
...
describe("Contexto finalizando tarefa", function () {
  ...
  test.skip("Deve finalizar uma tarefa que esteja em execução", async function () {
    ...
    const sut = new CancellingTask(accountRepository, taskRepository, getTask);
    const [data] = await connection.query(
      'SELECT external_id FROM task WHERE status = "Done"',
      []
    );
    const input = {
    ...
    expect(result).toBe("Task canceled");
  });
